...
use casper_types::{
    contracts::NamedKeys, ApiError, CLType, ContractPackageHash, EntryPoint, EntryPointAccess,
    EntryPointType, EntryPoints, Parameter, URef,
};
use contract::Threebonacci;
...
fn fib() {
    ...
    let threebonacci: u64 = Threebonacci::new()
        .nth(
            n.try_into()
    ...
    let val = storage::new_uref(threebonacci);
...
pub extern "C" fn call() {
    ...
    let contract_package_hash: ContractPackageHash =
        runtime::get_key(CONTRACT_PACKAGE_HASH_NAMED_KEY)
            .unwrap_or_revert()
            .into_hash()
            .unwrap()
            .into();

    let _contract_package_access: URef = runtime::get_key(CONTRACT_PACKAGE_ACCESS_NAMED_KEY)
        .unwrap_or_revert()
        .try_into()
        .unwrap_or_revert();
